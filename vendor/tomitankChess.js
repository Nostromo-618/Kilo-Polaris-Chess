...
var EvalHashPack    = new Int32Array(EVAL_ENTRIES);
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	function Evaluation() {
		...
		if (EvalHashLock[evalHashIndex] === whitePovKeyHigh) {
			var entry = EvalHashPack[evalHashIndex];
			var score = entry >> 16;
			var side  = entry & 0x8;
			var symm  = entry & 0x1;
	...
	// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
		...
		var tempo = Interpolation(TempoMG, TempoEG, phase);

		score = Interpolation(MG_SC(score), EG_SC(score), phase);

		score += CurrentPlayer === WHITE ? tempo : -tempo; // +tempo
		...
		if (drawMul >= 0.5) score += ChessNNEval(); // NN ertekeles

		score = score * drawMul | 0; // Kerekites + ketes dontetlen..
		tempo = tempo * drawMul | 0;

		EvalHashLock[evalHashIndex] = whitePovKeyHigh; // check force: symmetry-safe (skip unstoppable passer)
		EvalHashPack[evalHashIndex] = (score << 16) | (tempo << 5) | CurrentPlayer | (wForce > 0 && bForce > 0);

		return (CurrentPlayer === WHITE ? score : -score) + tempo;
	}
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	function InitEnginSearch() {
		...
		EvalHashPack.fill(0); EvalHashLock.fill(0);
			...
			for (var msgNum = 0; msgNum < msgList.length; msgNum++)
			{
				...
				// ############################################################################################
					...
					case 'go':
						...
						if (MinSearchTime === 0) // Smart search..
						{
							...
							{
								MaxSearchTime = MinSearchTime = 1000 * 3600; // 1 hour
							}
						}
						else // Fix ido..
						{
				...
				// ############################################################################################
					...
					case 'isready':
						...
						setTimeout(function() { sendMessage('readyok'); }, 0); // Hack: wait for InitEvalMask
